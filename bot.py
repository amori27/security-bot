...
AUDIO_FORMAT = pyaudio.paInt16 if pyaudio is not None else None
CHANNELS = 1
RATE = 44100
CHUNK = 1024
VIDEO_FPS = 20
...
heatmap: Any = np.zeros((FRAME_HEIGHT, FRAME_WIDTH), dtype=np.float32)
...
def cleanup_old_files() -> None:
    ...
    cutoff = now_dt - datetime.timedelta(days=FILE_RETENTION_DAYS)
...
def capture_photo(frame: np.ndarray | None = None) -> None:
    folder, _, _ = make_paths("photo")
    path = os.path.join(
        folder,
        f"alert_{datetime.datetime.now().strftime('%H%M%S')}.jpg",  # noqa: DTZ005
    )
    if frame is None:
        with lock:
            ret, frame = cap.read()
...
def play_stream_in_vlc(url: str) -> None:
    def _play() -> None:
        try:
            ...
            send_telegram_message(f"❌ Playback failed: {e!s}")
...
class VideoRecorder(threading.Thread):
    def __init__(self, path: str):
        ...
        self.writer = cv2.VideoWriter(
            path,
            getattr(cv2, "VideoWriter_fourcc")(*"mp4v"),  # noqa: B009
            VIDEO_FPS,
            FRAME_SIZE,
        )
        self.duration = 0.0
...
def automatic_motion_loop() -> None:
    ...
    global auto_video_path, auto_audio_path, motion_count, motion_start_time
    ...
    while True:
        ...
        for c in contours:
            ...
            if area > MOTION_THRESHOLD:
                ...
                cv2.rectangle(
                    display_frame, (x, y), (x + w, y + h), BOX_COLOR, BOX_THICKNESS
                )
                cv2.putText(
                    display_frame,
                    f"ALERT {area}px",
        ...
        ts_text = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # noqa: DTZ005
        cv2.putText(
            display_frame,
            ts_text,
            (10, FRAME_HEIGHT - 20),
...
def telegram_listener() -> None:
    ...
    while True:
        try:
            ...
            for u in (
                requests.get(url, params=params, timeout=35).json().get("result", [])
            ):
                last_id = u["update_id"]
                msg = u.get("message", {})
                if str(msg.get("chat", {}).get("id")) == TELEGRAM_CHAT_ID:
                    ...
                    if txt:
                        send_telegram_message(
                            handle_command(txt), parse_mode="Markdown"
                        )
